...
def my_model(width, height, output):
    model = tf.keras.models.Sequential()

    model.add(tf.keras.layers.Conv2D(96, 11, input_shape=(width, height, 3), activation='relu'))
    model.add(tf.keras.layers.MaxPooling2D(3, strides=4))
    model.add(tf.keras.layers.LayerNormalization())
    model.add(tf.keras.layers.Conv2D(256, 4, padding="same", activation='relu'))
    model.add(tf.keras.layers.MaxPooling2D(2, strides=2))
    model.add(tf.keras.layers.Conv2D(384, 3, padding="same", activation='relu'))
    model.add(tf.keras.layers.Conv2D(384, 3, padding="same", activation='relu'))
    model.add(tf.keras.layers.Conv2D(384, 3, padding="same", activation='relu'))
