...
def getSqueezeNetModel(x, y, keep_prob, learning_rate=1e-3):
    ...
    def squeeze(inputs, squeezeTo):
        with tf.name_scope('squeeze'):
            ...
            w = tf.Variable(tf.compat.v1.truncated_normal([1, 1, inputSize, squeezeTo]))
    ...
    def expand(inputs, expandTo):
        with tf.name_scope('expand'):
            ...
            w = tf.Variable(tf.compat.v1.truncated_normal([1, 1, squeezeTo, expandTo]))
            h1x1 = tf.nn.relu(tf.nn.conv2d(inputs, w, [1, 1, 1, 1], 'SAME'))
            w = tf.Variable(tf.compat.v1.truncated_normal([3, 3, squeezeTo, expandTo]))
    ...
    with tf.name_scope('conv1'):
        w = tf.Variable(tf.compat.v1.truncated_normal([3, 3, 1, 64]))
    ...
    with tf.name_scope('conv10'):
        ...
        w = tf.Variable(tf.compat.v1.truncated_normal([1, 1, input_shape, classCount]))
